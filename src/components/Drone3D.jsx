...
export default function Drone3D({ 
  ...
  useFrame((state, delta) => {
    ...
    // Apply realistic rotation from parent component (pitch, roll, yaw in degrees)
    if (droneRef.current) {
      // Rotation format: [pitch, roll, yaw] in degrees
      // Convert to radians for Three.js
      const pitchRad = (rotation[0] * Math.PI) / 180;
      const rollRad = (rotation[1] * Math.PI) / 180;
      const yawRad = (rotation[2] * Math.PI) / 180;
      
      // Apply smooth interpolation for realistic movement
      const smoothing = 0.12;
      const currentRotation = droneRef.current.rotation;
      
      // Pitch (X-axis) - nose up/down (forward tilt)
      const pitchDiff = pitchRad - currentRotation.x;
      currentRotation.x += pitchDiff * smoothing;
      
      // Roll (Z-axis) - tilt left/right (lateral tilt)
      const rollDiff = rollRad - currentRotation.z;
      currentRotation.z += rollDiff * smoothing;
      
      // Yaw (Y-axis) - rotate left/right (heading)
      // Handle wrap-around for continuous rotation
      let yawDiff = yawRad - currentRotation.y;
    ...
    if (armed || flying) {
      ...
      // Realistic RPM: idle ~3000, max ~15000 RPM -> convert to rad/s
      const idleSpeed = 5;  // rad/s at idle
      const maxSpeed = 80;  // rad/s at full throttle
      const baseSpeed = flying ? idleSpeed + (throttle / 100) * (maxSpeed - idleSpeed) : idleSpeed;
      
      propellerRefs.forEach((ref, index) => {
        if (ref.current) {
          // Quadcopter motor pattern: CW/CCW alternating for stability
          // Front-right(0): CW, Front-left(1): CCW, Back-left(2): CW, Back-right(3): CCW
          const direction = index % 2 === 0 ? 1 : -1;
          
          // Add realistic per-motor variation (±3%)
          const variation = 1 + (Math.sin(time * 2 + index) * 0.03);
          const rotationSpeed = baseSpeed * variation * delta * direction;
          
          ref.current.rotation.y += rotationSpeed;
          
          // Blur effect at high speeds - realistic motion blur
          const speedRatio = throttle / 100;
          const blurAmount = speedRatio * 0.7;
          const targetOpacity = flying ? Math.max(0.25, 1 - blurAmount) : 0.98;
          
          // Apply opacity to blade children (meshes) for motion blur effect
      ...
      // When disarmed, gradually stop propellers with friction
      propellerRefs.forEach((ref) => {
        if (ref.current) {
          // Realistic deceleration with angular friction
          const friction = 0.90;
          ref.current.rotation.y *= friction;
          
          // Reset blade opacity to solid (no blur when stopped)
          ref.current.children.forEach(child => {
            if (child.material && child.material.opacity !== undefined) {
              child.material.opacity += (1 - child.material.opacity) * 0.06;
              child.material.needsUpdate = true;
            }
          });
        }
    ...
    if (flying && droneRef.current) {
      // Small altitude oscillations from motor PID micro-adjustments (±3mm)
      const hoverBob = Math.sin(time * 3.5) * 0.003 + Math.sin(time * 5.2) * 0.0015;
      droneRef.current.position.y = hoverBob;
      
      // Micro pitch/roll adjustments from wind and stabilization (±0.5 degrees)
      const microPitch = Math.sin(time * 2.8) * 0.0005;
      const microRoll = Math.cos(time * 3.3) * 0.0005;
      droneRef.current.rotation.x += microPitch;
      droneRef.current.rotation.z += microRoll;
    } else if (droneRef.current) {
      // When not flying, ensure position is zeroed (no drift)
      droneRef.current.position.y = 0;
    }
    
    // Animate morphing arms - smooth servo-like transition to target positions
    const morphPositions = getMorphPositions();
    morphingArmRefs.forEach((ref, index) => {
      if (ref.current) {
        const target = morphPositions[index];
        // Smoother morphing with realistic servo speed (0.1 = ~2 seconds for full transition)
        const smoothing = 0.10;
        
        // Position interpolation (X and Z in meters)
        const posX = ref.current.position.x;
        const posZ = ref.current.position.z;
        ref.current.position.x += (target.x - posX) * smoothing;
        ref.current.position.z += (target.z - posZ) * smoothing;
        
        // Rotation interpolation (handle wrap-around)
        let angleDiff = target.angle - ref.current.rotation.y;
        while (angleDiff > Math.PI) angleDiff -= Math.PI * 2;
        while (angleDiff < -Math.PI) angleDiff += Math.PI * 2;
        ref.current.rotation.y += angleDiff * smoothing;
      }
    });
  });
